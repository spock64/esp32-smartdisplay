...
{
  ST7789_send_init_commands(CMD_SWRESET); // Software reset
...
// See ST7735_Setup.h file for an alternative format
  ST7789_send_init_commands(CMD_SLPOUT);   // Sleep out
  delay(120);

  ST7789_send_init_commands(CMD_NORON);    // Normal display mode on

  //------------------------------display and color format setting--------------------------------//
  ST7789_send_init_commands(CMD_MADCTL, [TFT_MAD_COLOR_ORDER], 1);
  ...
  ST7789_send_init_commands(CMD_RAMCTRL, [0x00, 0xE0], 2);
  //writedata(0x00);
  //writedata(0xE0); // 5 to 6 bit conversion: r0 = r5, b0 = b5

  ST7789_send_init_commands(CMD_COLMOD, [0x55], 1);
  //writedata(0x55);
  delay(10);

  //--------------------------------ST7789V Frame rate setting----------------------------------//
  ST7789_send_init_commands(CMD_PORCTRL, [0x0c, 0x0c, 0x00, 0x33, 0x35], 5);
  // writedata(0x0c);
  // writedata(0x0c);
  // writedata(0x00);
  // writedata(0x33);
  ...
  ST7789_send_init_commands(CMD_GCTRL, [0x35], 1);      // Voltages: VGH / VGL
  //writedata(0x35);

  //---------------------------------ST7789V Power setting--------------------------------------//
  ST7789_send_init_commands(CMD_VCOMS, [0x28], 1);
  //writedata(0x28);		// JLX240 display datasheet

  ST7789_send_init_commands(CMD_LCMCTRL, [0x0c], 1);
  //writedata(0x0C);

  ST7789_send_init_commands(CMD_VDVVRHEN, [0x01, 0xFF], 2);
  // writedata(0x01);
  // writedata(0xFF);

  ST7789_send_init_commands(CMD_VRHS, [0x10], 1);       // voltage VRHS
  //writedata(0x10);

  ST7789_send_init_commands(CMD_VDVSET, [0x20], 1);
  //writedata(0x20);

  ST7789_send_init_commands(CMD_FRCTR2, [0x0f], 1);
  //writedata(0x0f);

  ST7789_send_init_commands(CMD_PWCTRL1, [0xa4, 0xa1], 2);
  // writedata(0xa4);
  // writedata(0xa1);

  //--------------------------------ST7789V gamma setting---------------------------------------//
  ST7789_send_init_commands(CMD_PVGAMCTRL, [0xd0, 0x00, 0x02, 0x07, 0x0a, 0x28, 0x32, 0x44, 0x42, 0x06, 0x12, 0x14, 0x17], 14);
  // writedata(0xd0);
  // writedata(0x00);
  // writedata(0x02);
  // writedata(0x07);
  ...
  ST7789_send_init_commands(CMD_NVGAMCTRL, [0xd0, 0x00, 0x02, 0x07, 0x0a, 0x28, 0x31, 0x54, 0x47, 0x0e, 0x1c, 0x17, 0x1b, 0x1e], 14);
  // writedata(0xd0);
  // writedata(0x00);
  // writedata(0x02);
  // writedata(0x07);
  ...
  ST7789_send_init_commands(CMD_INVON);

  ST7789_send_init_commands(CMD_CASET, [0x00, 0x00, 0x00, 0xef], 4);    // Column address set
  // writedata(0x00);
  // writedata(0x00);
  // writedata(0x00);
  // writedata(0xEF);    // 239

  ST7789_send_init_commands(CMD_RASET, [0x00, 0x00, 0x01, 0x3f]);    // Row address set
  // writedata(0x00);
  // writedata(0x00);
  // writedata(0x01);
  // writedata(0x3F);    // 319
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  ST7789_send_init_commands(ST7789_DISPON);    //Display on
  delay(120);


#ifdef TFT_INVERSION_ON
  ST7789_send_init_commands(TFT_INVON);
#endif

#ifdef TFT_INVERSION_OFF
  ST7789_send_init_commands(TFT_INVOFF);
#endif
